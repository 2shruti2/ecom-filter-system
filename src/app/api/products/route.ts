...
const AVG_PRODUCT_PRICE = 500 as const;
const MAX_PRODUCT_PRICE = 1000 as const;

export const POST = async (req: NextRequest) => {
  try {
    const body = await req.json(); // sending req in json format
    ...
    const products = await db.query({
      topK: 12, // how many results we want to show
      vector: [
        0,
        0,
        sort === "none"
    ...
    console.log(error); // or we can log error to sentry
